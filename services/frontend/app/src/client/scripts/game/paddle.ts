...
export function createKeyDownEvent(ws: WebSocket) {
    return function keyDownEvent(event: KeyboardEvent): void {
        ...
        if (event.key === 'ArrowUp') updatePaddlePos(game.rightPad, 'Pad: ArrowUp', ws, ctx);
        if (event.key === 'ArrowDown') updatePaddlePos(game.rightPad, 'Pad: ArrowDown', ws, ctx);
        if (event.key === 'w') updatePaddlePos(game.leftPad, 'Pad: w', ws, ctx);
        if (event.key === 's') updatePaddlePos(game.leftPad, 'Pad: s', ws, ctx);
    };
}
