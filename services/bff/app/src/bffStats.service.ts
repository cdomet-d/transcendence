...
export async function fetchUserStats(userID: number): Promise<Response> {
	const url = `http://users-service:3000/internal/users/${userID}/stats`;
	return fetch(url);
}
...
export async function incrementWinStreak(userID: number): Promise<Response> {
	const url = `http://users-service:3000/internal/users/${userID}/stats/win-streak/increment`;
	return fetch(url, {
		method: 'POST'
	});
}
...
export async function resetWinStreak(userID: number): Promise<Response> {
	const url = `http://users-service:3000/internal/users/${userID}/stats/win-streak/reset`;
	return fetch(url, {
		method: 'POST'
	});
}
