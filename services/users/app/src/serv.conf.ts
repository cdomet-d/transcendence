...
function getBffIP(): string | null {
    ...
    if (ip === undefined) throw new Error('BFF IP is undefined');
    dns.lookup(ip, (err, address) => {
        if (err) throw new Error('failed to resolve BFF IP address');
        return address;
    });
    return null;
}
...
function getAuthIP(): string | null {
    ...
    if (ip === undefined) throw new Error('AUTH IP is undefined');
    dns.lookup(ip, (err, address) => {
        if (err) throw new Error('failed to resolve auth IP address');
        return address;
    });
    return null;
}
...
function checkProxy(address: string, hop: number): boolean {
    ...
    if ((address === nginxIP || address === bffIP || address === authIP) && hop === 3) return true;
    return false;
}
