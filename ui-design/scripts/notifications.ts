...
if (clickedDiv) {
  clickedDiv.addEventListener("click", () => {
    const notificationInteractiveWrapper = document.getElementById(
      "notificationInteractiveWrapper"
    );
    const unreadNotificationAlert = document.getElementById(
    ...
    if (notificationInteractiveWrapper) {
      ...
      else notificationInteractiveWrapper.setAttribute("selected", "");
    }
